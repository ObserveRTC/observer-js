	...
	const process = async (transaction: TransactionContext) => {
		...
		for (const deletedPeerConnectionId of deletedPeerConnections) {
			if (updatedPeerConnections.has(deletedPeerConnectionId)) {
				logger.warn(`Updated PeerConnection held a peerConnectionId for deletedPeerConnections. Transaction ${transaction.id}`);
		...
		for (const deletedInboundTrackId of deletedInboundAudioTracks) {
			if (updatedInboundAudioTracks.has(deletedInboundTrackId)) {
				logger.warn(`Updated InboundAudioTrack held an inboundAudioTrackId for deletedInboundAudioTracks. Transaction ${transaction.id}`);
		...
		for (const deletedInboundTrackId of deletedInboundVideoTracks) {
			if (updatedInboundVideoTracks.has(deletedInboundTrackId)) {
				logger.warn(`Updated InboundVideoTrack held an inboundVideoTrackId for deletedInboundVideoTracks. Transaction ${transaction.id}`);
		...
		for (const deletedOutboundTrackId of deletedOutboundAudioTracks) {
			if (updatedOutboundAudioTracks.has(deletedOutboundTrackId)) {
				logger.warn(`Updated OutboundAudioTrack held an outboundAudioTrackId for deletedOutboundAudioTracks. Transaction ${transaction.id}`);
		...
		for (const deletedOutboundTrackId of deletedOutboundVideoTracks) {
			if (updatedOutboundVideoTracks.has(deletedOutboundTrackId)) {
				logger.warn(`Updated OutboundVideoTrack held an outboundVideoTrackId for deletedOutboundVideoTracks. Transaction ${transaction.id}`);
		...
		for (const deletedSfuTransportId of deletedSfuTransports) {
			if (updatedSfuTransports.has(deletedSfuTransportId)) {
				logger.warn(`Updated SfuTransport held an sfuTransportId for deletedSfuTransports. Transaction ${transaction.id}`);
		...
		for (const deletedSfuInboundRtpPadId of deletedSfuInboundRtpPads) {
			if (updatedSfuInboundRtpPads.has(deletedSfuInboundRtpPadId)) {
				logger.warn(`Updated SfuInboundRtpPad held an updatedSfuInboundRtpPadId for deletedSfuInboundRtpPads. Transaction ${transaction.id}`);
		...
		for (const deletedSfuOutboundRtpPadId of deletedSfuOutboundRtpPads) {
			if (updatedSfuOutboundRtpPads.has(deletedSfuOutboundRtpPadId)) {
				logger.warn(`Updated SfuOutboundRtpPad held an updatedSfuOutboundRtpPadId for deletedSfuOutboundRtpPads. Transaction ${transaction.id}`);
		...
		for (const deletedSfuSctpChannelId of deletedSfuSctpChannels) {
			if (updatedSfuSctpChannels.has(deletedSfuSctpChannelId)) {
				logger.warn(`Updated SfuSctpChannelId held an deletedSfuSctpChannelId for deletedSfuSctpChannels. Transaction ${transaction.id}`);
				updatedSfuSctpChannels.delete(deletedSfuSctpChannelId);
			}
		}
		
		const promiseSetAll = <K, V>(
		  storage: ObserverStorage<K, V>,
		...
		evaluatorContext.closedPeerConnections.push(...Array.from(removedPeerConnectionModels.values()).map(o => {
			return {
				...o,
				closed: Date.now(),
			}
		...
		evaluatorContext.addedInboundAudioTrackIds.push(...addedUnits(oldInboundAudioTrackModels, updatedInboundAudioTracks));
		
		evaluatorContext.removedInboundVideoTracks.push(...removedUnits(removedInboundVideoTrackModels));
		evaluatorContext.addedInboundVideoTrackIds.push(...addedUnits(oldInboundVideoTrackModels, updatedInboundVideoTracks));
		
		evaluatorContext.removedOutboundAudioTracks.push(...removedUnits(removedOutboundAudioTrackModels));
		evaluatorContext.addedOutboundAudioTrackIds.push(...addedUnits(oldOutboundAudioTrackModels, updatedOutboundAudioTracks));
		
		evaluatorContext.removedOutboundVideoTracks.push(...removedUnits(removedOutboundVideoTrackModels));
		evaluatorContext.addedOutboundVideoTrackIds.push(...addedUnits(oldOutboundVideoTrackModels, updatedOutboundVideoTracks));
	};
	  
	const result = async (context: TransactionContext, next?: Middleware<TransactionContext>) => {
		await Promise.all([
			process(context),
			next ? next(context) : Promise.resolve()
		]);
	};
	return result;
}

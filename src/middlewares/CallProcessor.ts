...
export function createCallProcessor(
	...
	const joinClientProcess = createJoinClientProcess(
		storageProvider,
		reportsCollector,
	);
	const detachClientProcess = createDetachClientProcess(
		storageProvider,
		reportsCollector,
	);
	...
	const detachClientMiddleware: Middleware<CallOperationsContext> = async (context, next) => {
		...
		await Promise.all([
			detachClientProcess(input),
			next ? next(context) : Promise.resolve(),
		]);
	};

	return createProcessor(
		joinClientMiddleware,
		detachClientMiddleware
	);
}
...
export function createDetachClientProcess(
	...
	const peerConnectionClosedProcess = createClosePeerConnectionProcess(
		storageProvider,
	);
	const process = async (context: DetachClientProcessInput) => {
		...
		for (const client of clients.values()) {
			...
			callToClientIds.set(
				callId,
				[...(callToClientIds.get(callId) ?? []), clientId]
			);
		}
		...
		await clientStorage.removeAll(clientIds).then(deletedClientIds => {
			for (const { clientId, detached } of detachedClients) {
				...
				reports.addCallEventReport(createClientLeftEventReport(
					serviceId,
					mediaUnitId,
					roomId,
					callId,
				...
				callDetaches.set(
					callId,
					Math.max((callDetaches.get(callId) ?? 0), detached)
				);
			}
		});
		...
		if (0 < removedCallIds.length) {
			await callStorage.removeAll(removedCallIds).then(deletedCalls => {
				for (const [callId, deletedCall] of deletedCalls) {
					...
					if (!serviceId || !roomId || !callId) 
						continue;

					const ended = callDetaches.get(callId) ?? Date.now();
					const callEvent = createCallEndedEventReport(
						serviceId,
						roomId,
						callId,
						ended,

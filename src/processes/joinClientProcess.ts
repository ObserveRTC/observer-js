...
export function createJoinClientProcess(
	...
	const process = async (context: JoinClientProcessInput) => {
		...
		for (const joinedClient of joinedClients) {
			...
			newClients.set(clientId, new Models.Client({
				serviceId,
				mediaUnitId,
				roomId,
				callId,
		...
		const alreadyInsertedCalls = 0 < newCalls.size 
			? (await callStorage.insertAll(newCalls)) 
			: new Map<string, Models.Call>();
		...
		for (const [callId, newCall] of newCalls) {
			...
			const callEvent = createCallStartedEventReport(
				serviceId,
				roomId,
				callId,
				Number(started),

...
| [`detectors.ts`](./detectors.ts) | The reference. Each of the eight detectors with the question it answers, its full config, a scenario that makes it fire, and the finding it produces. |
...
Note what is **not** printed: no `[sfu-wide]` finding. `ConcurrentIssueDetector` also runs at
observer scope, but there it requires the affected clients to span at least two *independent* calls
before concluding anything — and this demo has one. That is the point of the gate: one bad meeting is
the call-scoped detector's business, and raising a fleet alert for it would be a false positive.
Feed a second call with the same issue open and `CROSS_CALL_ISSUE_ONSET_BURST` appears, carrying a
`conclusion` of `faultDomain: 'infrastructure'`.

Three details in the file worth copying into real integrations:

- **Detectors are configured, not registered.** The example passes `callDetectors` / `detectors` to
  the `Observer` constructor; each slot takes overrides, or `null` to skip that detector. Omitting
  both keys entirely gives every detector with defaults.
...
|---|----------|----------|--------|
| 1 | `ConcurrentIssueDetector` (call) | 3 of 4 participants congested at once | `ISSUE_ONSET_BURST` |
| 2 | `ConcurrentIssueDetector` (observer) | 1 congested client in each of 3 unrelated calls | `CROSS_CALL_ISSUE_ONSET_BURST` |
...
| 7 | `IceDisruptionDetector` | 3 of 4 lose ICE in the same window | `CALL_ICE_DISRUPTION` |
...
Each scenario disables every other detector (`observerDetectors: null, callDetectors: null`, then one
explicit `add`). That is *not* how to configure production — there you leave them on and switch off
what you don't want, which the final section shows — but it makes each finding traceable to one
detector. The script asserts all eight findings fired, and that the validator settles, and exits non-zero otherwise, so it doubles as a smoke
test.
